  ...
  return <div className="page estate-dashboard">
    ...
    <section className="dashboard-charts" aria-label="Estate insights">
      ...
      <article className="dashboard-panel dashboard-working-days-panel">
        ...
        <div className="dashboard-working-days-list">{workerDayRows.length ? workerDayRows.map(({ worker, days, weeks, gross, takeHome }, index) => <article key={worker.id}>
          <header className="dashboard-worker-heading"><span className="dashboard-worker-number" aria-hidden="true">{String(index + 1).padStart(2, '0')}</span><div><h3>{worker.name}</h3><p>{weeks} saved {weeks === 1 ? 'week' : 'weeks'}</p></div></header>
          <dl className="dashboard-worker-figures"><div className="worker-days"><dt>Days worked</dt><dd>{quantity(days)}</dd></div><div><dt>Gross pay</dt><dd>{money(gross)}</dd></div><div className="worker-net"><dt>Take-home pay</dt><dd>{money(takeHome)}</dd></div></dl>
        </article>) : <p className="dashboard-working-days-empty">No saved worker payments in this period.</p>}</div>
      </article>
