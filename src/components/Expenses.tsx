...
import { expensesByCategory, money, yearlyExpenseTotal, yearlyLabourTotal } from '../lib/calculations'
...
export function Expenses({ data, year, refresh }: { data: EstateData; year: number; refresh: () => Promise<void> }) {
  ...
  const labourCost = yearlyLabourTotal(data.weeklyPayments, year)
  const nonLabourCost = yearlyExpenseTotal(data.expenses, year)
  ...
  return <div className="page space-y-5"><header><p className="text-sm font-bold uppercase tracking-wider text-leaf-700">Costs</p><h1 className="mt-1 text-3xl font-extrabold">Expense tracker</h1><p className="mt-1 text-stone-600">Track non-labour costs here; labour cost is included automatically in the estate total.</p></header>
    ...
    <section className="grid grid-cols-2 gap-3 xl:grid-cols-4"><SmallTotal label="This month (non-labour)" value={money(monthlyTotal)} tone="expense" /><SmallTotal label="Labour cost" value={money(labourCost)} tone="labour" /><SmallTotal label="Total incl. labour" value={money(nonLabourCost + labourCost)} tone="revenue" /><SmallTotal label="Categories" value={String(activeCategories.length)} tone="category" /></section>
