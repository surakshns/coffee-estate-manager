...
export function Labour({ data, year, refresh }: { data: EstateData; year: number; refresh: () => Promise<void> }) {
  ...
  return <div className="page space-y-5">
    ...
    <section className="grid gap-3 sm:grid-cols-3"><SummaryCard label={`Labour cost in ${year}`} value={money(paidThisYear)} detail="Included automatically in total estate expenses." /><SummaryCard label="Outstanding worker loans" value={money(totalLoanBalance)} detail="Advances less recorded repayments." /><SummaryCard label="Weekly payment flow" value="Set amounts → remove anyone not paid → save once" detail="New active workers appear in all months and weeks." compact /></section>
...
function SummaryCard({ label, value, detail, compact = false }: { label: string; value: string; detail: string; compact?: boolean }) { return <div className="card"><p className="text-sm font-bold text-stone-600">{label}</p><p className={compact ? 'mt-1 text-base font-semibold' : 'mt-1 text-3xl font-extrabold'}>{value}</p><p className="mt-1 text-sm text-stone-500">{detail}</p></div> }
...
function WednesdayCard({ date, workers, payments, open, amounts, removed, setAmounts, setRemoved, onToggle, onSaveAll }: { date: string; workers: Worker[]; payments: EstateData['weeklyPayments']; open: boolean; amounts: Record<string, string>; removed: Record<string, boolean>; setAmounts: React.Dispatch<React.SetStateAction<Record<string, string>>>; setRemoved: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; onToggle: () => void; onSaveAll: () => void }) {
  ...
  return <div className="overflow-hidden rounded-xl border border-stone-200"><button className="flex min-h-15 w-full items-center justify-between gap-3 bg-stone-50 px-4 text-left" onClick={onToggle} aria-expanded={open}><span className="min-w-0"><span className="block text-base font-extrabold text-stone-900">{friendlyDate(date)}</span><span className="block text-sm text-stone-500">Labour cost: {money(isRecorded ? recordedTotal : suggestedTotal)}</span></span><span className="shrink-0 text-xl text-stone-400">{open ? '−' : '+'}</span></button>{open && <div className="space-y-3 p-3 sm:p-4">{paymentWorkers.length ? paymentWorkers.map((worker) => <div className={`flex flex-col gap-3 rounded-xl border border-stone-200 p-3 sm:flex-row sm:items-center ${removed[worker.id] ? 'bg-stone-50 opacity-70' : ''}`} key={worker.id}><span className="min-w-0 font-extrabold sm:min-w-42">{worker.name}</span><label className="flex flex-1 items-center gap-2 text-sm font-bold text-stone-600">Amount ₹<input className="field mt-0 max-w-45" inputMode="decimal" disabled={removed[worker.id]} value={amounts[worker.id] ?? ''} onChange={(event) => setAmounts((current) => ({ ...current, [worker.id]: event.target.value }))} /></label><button type="button" className="button-secondary" onClick={() => setRemoved((current) => ({ ...current, [worker.id]: !current[worker.id] }))}>{removed[worker.id] ? 'Restore this week' : 'Remove this week'}</button></div>) : <p className="rounded-xl bg-amber-50 p-3 text-amber-900">No active workers. Add a worker below.</p>}<div className="flex flex-col gap-3 border-t border-stone-200 pt-3 sm:flex-row sm:items-center"><button type="button" className="button-primary" onClick={onSaveAll} disabled={!paymentWorkers.length}>Save all payments</button><span className="text-sm font-bold text-stone-600">Labour cost for this week: {money(total)}</span></div></div>}</div>
}
