...
export function Labour({ data, year, refresh }: { data: EstateData; year: number; refresh: () => Promise<void> }) {
  ...
  const accordionAnimation = useRef<number | null>(null)
  ...
  useEffect(() => () => {
    if (accordionAnimation.current !== null) window.cancelAnimationFrame(accordionAnimation.current)
  }, [])

  function keepAccordionTriggerInView(button: HTMLButtonElement) {
    if (accordionAnimation.current !== null) window.cancelAnimationFrame(accordionAnimation.current)
    const initialTop = button.getBoundingClientRect().top
    const startedAt = performance.now()
    const keepPosition = (now: number) => {
      const offset = button.getBoundingClientRect().top - initialTop
      if (Math.abs(offset) > 0.5) window.scrollBy(0, offset)
      if (now - startedAt < 340) accordionAnimation.current = window.requestAnimationFrame(keepPosition)
      else accordionAnimation.current = null
    }
    accordionAnimation.current = window.requestAnimationFrame(keepPosition)
  }

  function toggleMonth(event: MouseEvent<HTMLButtonElement>, monthIndex: number) {
    keepAccordionTriggerInView(event.currentTarget)
    setOpenMonth((current) => current === monthIndex ? -1 : monthIndex)
    setOpenWednesday('')
  }

  function toggleWednesday(event: MouseEvent<HTMLButtonElement>, date: string) {
    keepAccordionTriggerInView(event.currentTarget)
  ...
  const startEditing = (worker: Worker) => {
    ...
    setWorkerAmount(String(worker.default_weekly_amount))
...
function AnimatedAccordionPanel({ open, children }: { open: boolean; children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      setExpanded(false)
      closeTimer = window.setTimeout(() => setRendered(false), 300)
    }
    return () => {
