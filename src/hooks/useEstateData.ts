...
const empty: EstateData = { workers: [], weeklyPayments: [], workerLoans: [], categories: [], expenses: [], prices: [], production: [], sales: [] }
...
export function useEstateData() {
  ...
  const refresh = useCallback(async () => {
    ...
    const firstError = [workers, weeklyPayments, workerLoans, categories, expenses, prices, production, sales].find((result) => result.error)?.error
